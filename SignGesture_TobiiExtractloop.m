%% Extracts trial information from Tobii file for Rain's Sign/Gesture Study
% Adam Stone, Jan 2018
%
...
if strfind(csvfile, 'GROUP1')
    group = 1;
elseif strfind(csvfile, 'GROUP2')
    group = 2;
else warning('Hello?')
end

signer = alldata.x_Signer_Value{1};
%analysis = alldata.x_Quality_Value{1};
...
for i = 1:length(calib)
    if strfind(events.MediaName{i}, 'ATTENTION')
        calib(i) = 1;
    end
    if strfind(events.MediaName{i}, 'Center')
        calib(i) = 1;
    end
end
events.calib = calib;
...
for i = 1:length(marks)
    if strfind(events.MediaName{i}, 'SignA')
        marks(i) = 1;
    elseif strfind(events.MediaName{i}, 'SignB')
        marks(i) = 2;
    elseif strfind(events.MediaName{i}, 'SignC')
        marks(i) = 3;
    elseif strfind(events.MediaName{i}, 'SignD')
        marks(i) = 4;
    elseif strfind(events.MediaName{i}, 'GestureA')
        marks(i) = 5;
    elseif strfind(events.MediaName{i}, 'GestureB')
        marks(i) = 6;
    elseif strfind(events.MediaName{i}, 'GestureC')
        marks(i) = 7;
    elseif strfind(events.MediaName{i}, 'GestureD')
        marks(i) = 8;
    end
end
...
signers = repmat({signer},[length(aoidata),1]);
...
aoidata = [IDs,participants,groups,signers,aoidata];
rownames = {'recording','participant','group','signer','condition','mark','trial','repetition','onset','offset','aoi','hits'};
