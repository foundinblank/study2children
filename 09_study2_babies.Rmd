...
The chart below shows little differences based on factors AgeGroup, Language, or Direction. That's good, means the videos were equally engaging for all babies, right? 
...
# Discussion
Maybe more we can do here (heat maps, certainly).  
...
One thing I worry about is using age *without any interactions* and whether people will criticize us for that. Because age x language x direction is a "more complete" model as it suggests age may differently influence various combinatiosn (SE + reversed, NSE + forward, etc) differently, while age + language x direction means we're mainly using age as a covariate, not as a factor of interest. But if we use age as a factor of interest, all significance disappears, so ... we want to use age as a covariate if we want to have a story. But how to defend that decision and not just be p-hacking? I have to think about it...