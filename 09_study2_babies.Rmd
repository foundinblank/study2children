...
participants <- xydata %>% 
  select(name, group, gender, language) %>% 
  distinct()

puppies <- left_join(participants, puppies, by = "name")

summary(lm(data = puppies, mean ~ language))

puppies_se <- filter(puppies, language == "SignLanguageExposed")
puppies_nse <- filter(puppies, language == "EnglishExposed")
...
summary(lmer(data = puppies, sec ~ language + (1|name)))

ggplot(puppies, aes(x = language, y = sec, fill = language)) + geom_boxplot()
```
