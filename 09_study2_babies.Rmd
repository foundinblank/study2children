...
A linear model shows there were no significant effects of any factors except for Trial order. We can conclude there was no effect found for age or language experience on how *much* the babies looked at the stimuli.  

```{r}
global_lm <- lmer(percent ~ age + lang * direction + trial + (1|name) + (1|story), data = babies_overall_looking)
summary(global_lm) 
confint(global_lm)
#ggcoef(global_lm)
...
babies_fcr <- babies %>%
  ungroup() %>%
  spread(aoi, percent) %>%
  group_by(name, age, agegroup, lang, gender, direction, story, trial) %>%
...
* Strong effect of language: SE babies have overall more FCR than NSE babies; in other words, they are more attracted to the face. 
* Strong effect of direction: Reversal *increases* FCR across all babies.
* Strong effect of trial: The more trials there are, the more FCR drops (fatigue?) or the more they're attending to hands 
* No language x direction interaction. 

```{r}
fcr_lm <- lmer(fcr ~ age + lang * direction + trial + (1|name) + (1|story), data = babies_fcr)
summary(fcr_lm)
confint(fcr_lm)

tab_model(fcr_lm, show.se = T, show.stat = T)

write_csv(babies_fcr, "fcr_trial_level_values_babies.csv")
...
# summary(fcrnse_lm)
```
## "Posthoc" for NSE babies only
```{r}

post_hoc <- lmer(fcr ~ age + direction + trial + (1|name) + (1|story), 
                 data = filter(babies_fcr, lang == 'NSE'))
summary(post_hoc)
confint(post_hoc)
```
...
babies_fcr2 %>%
    ...
    write_csv("fcr_individual_values_babies.csv")
